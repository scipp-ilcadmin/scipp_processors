...
void svm::init() { 
    ...
    _rootfile = new TFile("svm.root","RECREATE");
    ...
    _vector = new TH1F("vector","Vector Distribution",300,0,500);
...
void svm::processRunHeader( LCRunHeader* run) { 
//    _nRun++ ;
} 
void svm::processEvent( LCEvent * evt ) { 
    ...
    for(int i=0; i < col->getNumberOfElements(); ++i){
      ...
      double e=pow(particle->getEnergy(),2);
      double p=pow(mom[0],2)+pow(mom[1],2)+pow(mom[2],2);
      if (pid != 22){
	cout << sqrt(e-p) <<" "<< particle->getPDG()<< endl;
      }
