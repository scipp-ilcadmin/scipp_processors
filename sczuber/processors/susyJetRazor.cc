...
void susyJetRazor::init() { 
    ...
    if(_jetDetectability==0){_rootfile = new TFile("susyJetRazor_.39113._TRU0.5.root","RECREATE");
        ...
        _MRR2_TRU = new TH2F("MRR2_TRU","MRR2", 500, 0.01 ,100, 1000, 0.001, 1.4); // the M_{T}^{R} variable 
    ...
    if(_jetDetectability==1){_rootfile = new TFile("susyJetRazor_.39117._DAB0.5.root","RECREATE");
       ...
       // _beta_DAB = new TH1F("beta_DAB","beta",80,-20,20);
        ...
        freopen( "susyJetRazor_.39117._DAB0.5.log", "w", stdout ); 
    }
