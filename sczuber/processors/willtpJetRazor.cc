...
void tpJetRazor::processEvent( LCEvent * evt ) { 
    ...
    if(R>10){
        cerr << "FOUND EVENT WITH R>10!!"<< endl; 
        Rcheck += " ";
...
void tpJetRazor::end(){ 
    _rootfile->Write();
    cerr << "Events with R>10: " << Rcheck << endl;
}

vector<vector<PseudoJet>>* tpJetRazor::getMegajets(vector<PseudoJet> jets){
    // RAZOR MEGAJET ALGORITHM ------------------------------------------------
    vector<vector<PseudoJet>> *megajet;
    int num_partitions = pow(2, jets.size());  
    //double m2_values[num_partitions] = {0}; 
    //double min_value = 9999; // MIGHT BE PROBLEM IF *NO* JETS OR *ONE* JET!!
    double min_value = std::numeric_limits<double>::max(); // MIGHT BE PROBLEM IF *NO* JETS OR *ONE* JET!!
    int min_index = 0;

    for(int i = 0; i<num_partitions; i++){
        vector<vector<PseudoJet>>* partition = new vector<vector<PseudoJet>>(2);
        //vector<PseudoJet> partition = *_partition; 
        ...
        //
        if((*partition)[0].size() != 0 && (*partition)[1].size()!=0 && partition_m2<min_value ){
            min_index =i; 
            min_value = partition_m2;
            delete megajet;      
            megajet = partition; 
        }else{
            delete partition;
...
vector<vector<double>> tpJetRazor::getRframe4Vec(vector<double> j1, vector<double> j2){
    // boost into R-frame:  ------------------------------------------------------------------------
    //----------------------------------------------------------------------------------------------
